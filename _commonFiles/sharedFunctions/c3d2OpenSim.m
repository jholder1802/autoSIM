...
switch labFlag
    ...
    case 'PLUS_COD_noArms'
          % Rotation matrix from c3d to OpenSim for Markers
          R1 = rotx(0.5*pi);  % rotate 90째 around x
          R2 = roty(pi);      % rotate 180째 around y
          R3 = rotz(pi);      % rotate 180째 around z
          R1 = R1(1:3,1:3);   % remove translation
          R2 = R2(1:3,1:3);   % remove translation
          R3 = R3(1:3,1:3);   % remove translation
          R = R3*R2*R1;       % Total rotation
          ...
          % Rotation matrix force plate to coordinate system used in c3d file
          R_FPa = rotz(0.5*pi); % 90째 Drehung um die Z-Achse (Y wird zu -X, X wird zu Y)
          % R_FPb = rotz(0.5*pi);   % rotate 90째 z % Huth철fer --> urspr체nglich: 
          R_FPb = rotx(pi); % 180째 Drehung um die X-Achse (Z wird von negativ zu positiv umgekehrt)
          R_FPa = R_FPa(1:3,1:3); % remove translation
          R_FPb = R_FPb(1:3,1:3); % remove translation
          R_FP = R_FPa*R_FPb;     % Total rotation % % Reihenfolge ist wichtig, zuerst R_FPa, dann R_FPb

          % Rotate all force plates
          RotMat = rotz(0.5*pi);  % turn 90째 um Z - RotMat is a Matrix for individualised turning of each ForcePlates - in this case. R_FP1
...
%         R_FPc = R_FPc(1:3,1:3);
        ...
        % Rotate all force plates
        RotMat = rotz(1.5*pi);  % turn 270째 (ge채ndert von 90째) um Z - RotMat is a Matrix for individualised turning of each ForcePlates - in this case. R_FP1
        RotMat = RotMat(1:3,1:3);
        R_FP1 = R_FP*RotMat; 
        R_FP2 = R_FP*RotMat;  % *RotMat erg채nzt.
        % R_FP3 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP4 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP5 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP6 = R_FP; %
    ...
    case 'PLUS_SportsMarkerset_noArms_newFPs'
        ...
        % Rotation matrix force plate to coordinate system used in c3d file
        %{
        R_FPa = rotz(1.5*pi); % 90째 Drehung um die Z-Achse (Y wird zu -X, X wird zu -Y)
        R_FPb = rotx(pi); % 180째 Drehung um die X-Achse (Z wird von negativ zu positiv umgekehrt)
%         R_FPc = rotz(-0.5*pi); % erg채nzt jh
        R_FPa = R_FPa(1:3,1:3); % remove translation
        R_FPb = R_FPb(1:3,1:3); % remove translation
%         R_FPc = R_FPc(1:3,1:3);
        ...
        % Rotate all force plates
        RotMat = rotz(0.5*pi);  % turn 270째 (ge채ndert von 90째) um Z - RotMat is a Matrix for individualised turning of each ForcePlates - in this case. R_FP1
        RotMat = RotMat(1:3,1:3);
        R_FP1 = R_FP*RotMat; 
        R_FP2 = R_FP*RotMat;  % *RotMat erg채nzt.
        % R_FP3 = R_FP*RotMat;  % *RotMat erg채nzt.
        % R_FP3 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP4 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP5 = R_FP*RotMat(1:3,1:3);   % turn
        % R_FP6 = R_FP; %
